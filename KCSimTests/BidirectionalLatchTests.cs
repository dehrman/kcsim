...
namespace KCSimTests
{
    public class BidirectionalLatchTests
    {
        ...
        {
            bidirectionalLatch.InputPower.AddForce(new Force(1));
            bidirectionalLatch.InputControl.AddForce(new Force(input));
        ...
        public void TestThat_ControlValuesRemainLatchedAfterControlGoesToZero(int input, int expectedPositiveOutput, int expectedNegativeOutput)
        {
            ...
            bidirectionalLatch.InputControl.AddForce(new Force(input));
            bidirectionalLatch.InputControl.RemoveAllForces();
        ...
        {
            bidirectionalLatch.InputPower.AddForce(new Force(1));
            bidirectionalLatch.InputControl.AddForce(new Force(1));
            bidirectionalLatch.InputPower.RemoveAllForces();
